...
fn jvm_compareAndSwapObject(_jt: &mut JavaThread, _env: JNIEnv, args: Vec<Oop>) -> JNIResult {
    ...
    let v_at_offset = {
        ...
        match &v.v {
            oop::OopRef::Mirror(mirror) => mirror.field_values[offset as usize].clone(),
            oop::OopRef::Array(ary) => ary.elements[offset as usize].clone(),
            oop::OopRef::Inst(inst) => inst.field_values[offset as usize].clone(),
            t => unreachable!("{:?}", t),
        }
    };
    ...
    if util::oop::if_acmpeq(v_at_offset, old_data.clone()) {
        ...
        match &mut v.v {
            ...
            oop::OopRef::Inst(inst) => inst.field_values[offset as usize] = new_data.clone(),
            _ => unreachable!(),
        }
...
fn jvm_copyMemory(_jt: &mut JavaThread, _env: JNIEnv, args: Vec<Oop>) -> JNIResult {
    ...
    match src_obj {
        Oop::Null => {
            ...
            match &mut v.v {
                oop::OopRef::TypeArray(dest_ary) => match dest_ary {
